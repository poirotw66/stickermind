/**
 * Calls Gemini API using the user's API key from localStorage (Settings).
 * Key is never bundled; user adds it in the frontend Settings.
 */
import { GoogleGenAI } from '@google/genai';
import type { GenerationParams, StickerIdea, ThemeIdea } from '../types';
import { getStoredApiKey } from '../store/useApiKeyStore';
...
// Plain JSON Schema for structured output (works in web build)
...
function getAIClient(): GoogleGenAI {
  ...
  if (!apiKey) {
    throw new Error('請在「設定」中新增您的 Gemini API Key。');
  }
...
export async function generateStickerIdeas(params: GenerationParams): Promise<StickerIdea[]> {
  ...
  try {
    ...
    throw new Error('無法產生貼圖靈感，請稍後再試或檢查 API Key 是否正確。');
  }
}
...
export async function generateStickerThemes(params: GenerationParams): Promise<ThemeIdea[]> {
  ...
  try {
    ...
    const rawData = JSON.parse(response.text || '[]');
    return rawData.map((item: Record<string, unknown> & { examplePhrases: string[] }) => ({
      id: generateId(),
      title: item.title as string,
      description: item.description as string,
      sellingPoint: item.sellingPoint as string,
      examplePhrases: item.examplePhrases,
      createdAt: Date.now(),
    }));
  } catch (error) {
    console.error('Gemini Theme Generation Error:', error);
    throw new Error('無法產生主題靈感，請稍後再試或檢查 API Key 是否正確。');
  }
}
